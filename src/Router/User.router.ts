...
import { CountAccount, deleteUser, getProfile, getUserByID, getUserbyTeacher, getUserByUsername, SelectAcount, updateUser } from "../Controller/Auth.controller";
...
UserRouter.post('/deleteUser', async (req: AuthRequest, res: Response) => {
    const user = req.user
    const { id } = req.body
    if (user?.role == 'admin') {
        try {
            ...
            if (userDelete) {
                res.status(200).json({ message: 'Thành Công', result: Result.Succes, data: id })
            }
